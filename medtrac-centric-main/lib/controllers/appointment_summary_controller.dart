...
class AppointmentSummaryController extends GetxController {
  ...
  // Data holders
  final Rx<UserAppointmentDetails?> userAppointmentData =
      Rx<UserAppointmentDetails?>(null);
  final Rx<DoctorAppointmentDetails?> doctorAppointmentData =
      Rx<DoctorAppointmentDetails?>(null);
  ...
  Future<void> loadAppointmentDetails() async {
    try {
      ...
      if (isUser) {
        final response =
            await _patientService.getUserAppointmentDetails(appointmentId);
        ...
        final response =
            await _doctorService.getDoctorAppointmentDetails(appointmentId);
        if (response.status && response.data != null) {
  ...
  String get patientName => isUser
      ...
      : (doctorAppointmentData.value?.patient?.patientName ??
          'Unknown Patient');
